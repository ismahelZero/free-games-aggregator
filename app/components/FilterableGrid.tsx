...
export default function FilterableGrid({ games }: { games: any[] }) {
    ...
    const filteredGames =
        activeFilter === 'All' ? games : games.filter(g => g.platform === activeFilter)
    ...
    return (
        <div className="w-full">
            ...
            {/* The Bento Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 md:gap-6 auto-rows-[250px] md:auto-rows-[280px]">
                {filteredGames.length > 0 ? (
                    ...
                    <div className="col-span-full py-20 text-center bg-slate-800/20 rounded-3xl border border-dashed border-slate-700">
                        <p className="text-xl text-slate-500 font-bold">
                            No active loot found for this platform right now.
                        </p>
                    </div>
                )}
            </div>
